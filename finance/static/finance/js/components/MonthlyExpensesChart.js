...
(function(window) {
  ...
  const MonthlyExpensesChart = () => {
    ...
    useEffect(() => {
      ...
      document.addEventListener('periodChanged', handlePeriodChange);
      return () => {
        document.removeEventListener('periodChanged', handlePeriodChange);
      };
    }, []);
            ...
            <div className="col-md-4">
              <div className="card bg-light mb-3">
                <div className="card-body p-2 text-center">
                  ...
                  <p className="h4 mt-2 mb-0">
                    {(monthlyData.reduce((sum, item) => sum + item.value, 0) / monthlyData.length).toLocaleString(undefined, {maximumFractionDigits: 2})} د.أ
                  </p>
                </div>
              </div>
            </div>
